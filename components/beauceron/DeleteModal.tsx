  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/beauceron/delete?ids=${idsArray.join(",")}`,
        {
          method: "DELETE",
        }
      );
      ...
      if (data.success) {
        ...
        alert(data.message || "Failed to delete beauceron");
      }
    } catch (error) {
      console.error("Error deleting beauceron:", error);
      alert("An error occurred while deleting beauceron");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <AnimatePresence>
        ...
        >
          ...
          >
            {
              "Sind Sie sicher, dass Sie das ausgewählte Beauceron löschen möchten?"
            }
          </p>
